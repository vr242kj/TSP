...
int *insertion(int *a, int size, float **ary, int rows, int cols, float w[], int nrow){
	...
	for(int i = 0; i<size; i++){
		if(i < size-2){
			b[i] = get_weight(a[i],a[size-1],ary, rows, cols,w,nrow) + get_weight(a[size-1],a[i+1],ary, rows, cols,w,nrow) - get_weight(a[i+1],a[i],ary, rows, cols,w,nrow);
		}else{
			b[i] = get_weight(a[i],a[size-1],ary, rows, cols,w,nrow) + get_weight(a[size-1],a[0],ary, rows, cols,w,nrow) - get_weight(a[0],a[i],ary, rows, cols,w,nrow);
		}
		}
		min = b[0];
